...
$sqlExist = "SELECT * FROM usuarios WHERE usuario = ?";
if ($stmtExist = mysqli_prepare($con, $sqlExist)) {
    mysqli_stmt_bind_param($stmtExist, "s", $usuarioLogeado);
    mysqli_stmt_execute($stmtExist);
    mysqli_stmt_store_result($stmtExist);
    
    if (mysqli_stmt_num_rows($stmtExist) > 0) {
